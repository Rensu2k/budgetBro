...
const Budget_Detail_list = ({ route }) => {
  ...
  const { envId, title, refreshBudList, initializedDB, setInitializedDB } =
    route.params;
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedData = envelopeDetails
        .map((detail) => ({
          id: detail.id,
  ...
  const handleAddEntry = async () => {
    ...
    try {
      await DB.insertEnvelopeDetail(
        description,
        entryType,
        parseFloat(amount) || 0,
        entryDate.toISOString().split("T")[0],
        envId
      ); // Call insertEnvelopeDetail with user input
  ...
  const fetchRemainingIncome = async () => {
    try {
      ...
      setRemainingIncome(
        !isNaN(parsedIncome) ? parsedIncome.toFixed(2) : "0.00"
      );
    } catch (error) {
  ...
  const fetchRemainingAllocation = async () => {
    try {
      ...
      const parsedAllocation = parseFloat(allocation);
      setRemainingAllocation(
        !isNaN(parsedAllocation) ? parsedAllocation.toFixed(2) : "0.00"
      );
      setPossibleRemainingAllocation(
        !isNaN(parsedAllocation) ? parsedAllocation.toFixed(2) : "0.00"
      );
    } catch (error) {
