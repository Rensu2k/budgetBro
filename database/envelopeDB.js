...
export async function insertEnvelope(envName, dateAdded) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "INSERT INTO ENVELOPE_TABLE (ENV_NAME, DATE_ADDED) VALUES (?, ?)"
    );
...
export async function ImportinsertEnvelope(id, envName, dateAdded) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "INSERT INTO ENVELOPE_TABLE (ENV_ID,ENV_NAME, DATE_ADDED) VALUES (?, ?, ?)"
    );
...
export async function updateEnvelope(env_id, newTitle) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "UPDATE ENVELOPE_TABLE SET ENV_NAME = ? WHERE ENV_ID = ?"
    );
...
export async function deleteEnvelope(ENV_ID) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "DELETE FROM ENVELOPE_TABLE WHERE ENV_ID = ?"
    );
...
export async function insertIncome(amount, date, detail) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "INSERT INTO INCOME_TABLE (COLUMN_INCOME_AMOUNT, COLUMN_INCOME_DATE, COLUMN_INCOME_DETAIL) VALUES (?, ?, ?)"
    );
...
export async function updateIncome(id, amount, date, detail) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "UPDATE INCOME_TABLE SET COLUMN_INCOME_AMOUNT = ?, COLUMN_INCOME_DATE = ?, COLUMN_INCOME_DETAIL = ? WHERE ID = ?"
    );
...
export async function insertEnvelopeDetail(
  description,
  type,
  amount,
  date,
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "INSERT INTO ENVELOPE_DETAIL_TABLE (ENV_DETAIL_DESCRIPTION, DETAIL_TYPE, ENV_AMOUNT, ENV_DATE, ENV_ID) VALUES (?, ?, ?, ?, ?)"
    );
    const result = await statement.executeAsync([
      description,
      type,
...
export async function selectEnvelopeDetail(envId) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "SELECT * FROM ENVELOPE_DETAIL_TABLE WHERE ENV_ID = ?"
    );
...
export async function updateEnvelopeDetail(
  id,
  description,
  type,
  amount,
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "UPDATE ENVELOPE_DETAIL_TABLE SET ENV_DETAIL_DESCRIPTION = ?, DETAIL_TYPE = ?, ENV_AMOUNT = ?, ENV_DATE = ? WHERE ENV_DET_ID = ?"
    );
...
export async function deleteEnvelopeDetail(id) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "DELETE FROM ENVELOPE_DETAIL_TABLE WHERE ENV_DET_ID = ?"
    );
    await statement.executeAsync([id]);
  } catch (e) {
    console.error("error!:", e);
...
export async function deleteEnvelopeDetailByENV_ID(id) {
  ...
  try {
    const db = await SQLite.openDatabaseAsync(DATABASE_NAME);
    statement = await db.prepareAsync(
      "DELETE FROM ENVELOPE_DETAIL_TABLE WHERE ENV_ID = ?"
    );
    await statement.executeAsync([id]);
  } catch (e) {
    console.error("error!:", e);
